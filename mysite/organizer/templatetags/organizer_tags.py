...
def tagcloud(context, owner=None):
    url = reverse('organizer_lists_all')
    filters = {'link__is_public': True}

    if owner is not None:
        url = reverse('organizer_list_user',
            kwargs={'username': owner.username})
        filters['link__owner'] = owner
    if context['user'] == owner:
        del filters['link__is_public']

    tags = Tag.objects.filter(**filters)
    tags = tags.annotate(count=models.Count('link'))
